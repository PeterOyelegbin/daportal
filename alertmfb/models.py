...
class AccountOpening(models.Model):
    ...
    account_no = models.CharField(max_length=10, validators=[MinLengthValidator(10, 'minimum of 10 characters required')])
    branch = models.CharField(max_length=100)
    account_file = models.FileField(upload_to='account_files', validators=[acct_file_size])
    supervisor = models.ForeignKey(UserModel, related_name="AMFB_HOP", on_delete=models.CASCADE)
    upload_officer = models.ForeignKey(UserModel, related_name="AMFB_CSO", on_delete=models.PROTECT)
...
class LoanApplication(models.Model):
    ...
    approval_officer = models.ForeignKey(UserModel, related_name="AMFB_Credit_Officer", on_delete=models.CASCADE)
    upload_officer = models.ForeignKey(UserModel, related_name="AMFB_Loan_Officer", on_delete=models.PROTECT)
    approval_status = models.BooleanField(verbose_name="Approved", default="False")
    date_uploaded = models.DateField(auto_now_add=True)
