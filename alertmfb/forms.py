...
class AcctUploadForm(ModelForm):
    ...
    supervisor = ModelChoiceField(queryset=UserModel.objects.filter(role="HOP", organization="AMFB", is_active=True), widget=Select(attrs={"class":"form-control"}))
...
class LoanUploadForm(ModelForm):
    branch = ChoiceField(choices=BRANCH, widget=Select(attrs={"class":"form-control"}))
    approval_officer = ModelChoiceField(queryset=UserModel.objects.filter(role="Credit", organization="AMFB", is_active=True), widget=Select(attrs={"class":"form-control"}))
